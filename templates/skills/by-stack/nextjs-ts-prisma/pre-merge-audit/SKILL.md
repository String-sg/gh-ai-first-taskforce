...
description: Pre-merge audit for Next.js + TypeScript + Prisma projects. Checks marked **[automated]** are enforced by husky hooks — confirm the hooks are active, then focus on the manual checks below. Use before opening a PR.
...
- All user-supplied values reach the query via `$queryRaw` tagged template interpolation (`${value}`), `Prisma.sql`, or `Prisma.join`. Confirm the data-flow from request parsing through to the query parameters.
